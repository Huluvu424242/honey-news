import { d as filter, n as not, r as raceWith, g as argsOrArgArray } from './index-d3934a4e.js';
...
class EndpunkteService {
  constructor() {
    this.endpunkte = new Map();
  }
  /**
...
const endpunkteService = new EndpunkteService();
...
class Endpunkt {
  constructor(id, method, host, port, path, query) {
    this.id = id;
    this.method = method;
    this.host = host;
    this.port = port;
  ...
  replaceBase(urlBase, port) {
    const host = urlBase.replace(":" + port, "");
...
class BackendResponseImpl {
  constructor(fetchResponse, axiosResponse) {
...
class NetworkService {
  ...
  fetchDataFetchAPI(endpunkt) {
    return fetch(endpunkt.toUrl(), {
      method: endpunkt.getMethodAsString(),
      cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
    });
  }
  async fetchData(endpunkt) {
...
export { Endpunkt as E, FEED_PATH as F, Method as M, FEEDS_PATH as a, networkService as n };
