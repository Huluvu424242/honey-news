import { f as from, a as toArray, E as EMPTY, m as map, t as tap, s as switchMap, c as catchError, b as mergeMap, d as filter, l as lastValueFrom, e as timer } from './index-d3934a4e.js';
import { E as Endpunkt, M as Method, F as FEED_PATH, n as networkService } from './network-service-84df6987.js';
...
const ENDPOINT_NEWS = new Endpunkt("news", Method.GET, "https://huluvu424242.herokuapp.com", null, FEED_PATH, { statistic: true }).register();
/**
...
class NewsFetcher {
   ...
   * @param port port
   */
  static newNewsFetcherFor(host, port) {
    const endpunkt = ENDPOINT_NEWS.replaceBase(host, port);
    logService.logMessage("Define news endpunkt:" + endpunkt.toUrl());
    return new NewsFetcher(endpunkt);
  }
  loadFeedDataInternal(endpunkt) {
    const data = {
  ...
  async getFeedsSingleCall(feedURLs) {
    ...
    }), tap((post) => logService.debugMessage("### filter: " + post.item.title)), filter((post) => PipeOperators.compareDates(post.exaktdate, new Date()) < 1), toArray(), switchMap(
...
class NewsService {
  ...
  getFeedsPeriodicObservable$() {
    return timer(0, 60000 * 5).pipe(mergeMap(() => from(this.ladePosts()).pipe(catchError(() => EMPTY))));
  }
  // Fachlich relevant für CDC
  async ladePosts() {
    return await this.newsFetcher.getFeedsSingleCall(this.feedURLs);
  }
  async ladePostsFrom(url) {
...
export { PipeOperators as P, commonjsGlobal as a, createCommonjsModule as c, newsService as n };
