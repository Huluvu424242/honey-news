...
import {d as toArray, E as EMPTY, e as map, f as from} from './index-9062878f.js';
...
const HoneyNewsFeed = class {
  ...
  getPostEntry(post) {
    if (!post)
      return;
    return (h("honey-news-article", { post: post }));
  }
  getNeuesteMeldung() {
    var _a, _b;
    if (this.lastUpdate) {
  ...
  render() {
    return (h(Host, { title: this.getTitleText(), alt: this.getAltText(), tabindex: this.hasNoFeeds() ? -1 : this.taborder, disabled: this.hasNoFeeds() }, h("honey-apply-style", null), this.getNeuesteMeldung(), this.feeds.map((post) => [
